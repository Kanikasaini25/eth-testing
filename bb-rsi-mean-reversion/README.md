...
Standalone session-open bot for **ETH perpetual futures** on Delta Exchange testnet. It does **not** import, start, or change the existing LQDTY / YouTube backtester code.
...
| Item | Value |
|---|---|
| Timeframe | **15m** decides side. **5m** is the entry |
| Long | 2 green **15m** then 2 green **5m** back to back |
| Short | 2 red **15m** then 2 red **5m** back to back |
| Filters | Skip if SL distance **> 6 points**. No Bollinger / RSI |
| Session | **Opens only (IST):** London **12:30–2:00 PM**, US **7:00–9:30 PM** (both sides) |
| Re-entry | After a **loss**: wait **15 minutes**, then the same 15m + 5m pattern. Max **2 SL per open**. After a **profit**: wait for the **next open** |
| Daily cap | 4 trades per IST day |
| Kill-switch | Flatten + halt if daily PnL ≤ **-$14** |
| Stop | **100 lots**. Sell SL = first **5m** high (retry: one tick above). Buy SL = first **5m** low (retry: one tick below) |
...
The sidebar downloads **fresh** public **5m ETH** candles from Delta on every **Run backtest** click (no API key, no candle cache), then runs the same 15m/5m session-open rules, $3.50 stop sizing, 4-trade cap, and -$14 kill-switch.
...
Max window is 30 days of 5m data (~8,640 candles). India Live is recommended for historical candles.
