...
def _svg_bar_chart(
    ...
    *,
    ...
    for index, value in enumerate(values):
        ...
        bars.append(
            f'<rect x="{x:.1f}" y="{y:.1f}" width="{w:.1f}" height="{bar_height:.1f}" '
            f'fill="{color}" rx="2"/>'
        )
...
def render_cumulative_return_chart(backtest: BacktestResult) -> str:
    cumulative: list[float] = []
    running = 0.0
    for trade in backtest.trades:
        running += trade.return_pct
        cumulative.append(running)
    return _svg_line_chart(
        cumulative,
        stroke="#9467bd",
        title="Cumulative return % (per trade)",
        y_suffix="%",
    )


def render_trade_return_bars(backtest: BacktestResult) -> str:
    labels = [trade.entry_date[:10] for trade in backtest.trades]
    values = [trade.return_pct for trade in backtest.trades]
    return _svg_bar_chart(
        labels,
        values,
        title="Return % per trade — green = win, red = loss",
    )


def render_win_loss_summary(backtest: BacktestResult) -> str:
    wins = sum(1 for trade in backtest.trades if trade.return_pct > 0)
    losses = sum(1 for trade in backtest.trades if trade.return_pct < 0)
    breakeven = sum(1 for trade in backtest.trades if trade.return_pct == 0)
    total = max(len(backtest.trades), 1)
    ...
    <svg width="900" height="90" viewBox="0 0 900 90">
      <text x="4" y="16" fill="#666" font-size="12">
        Win/loss summary ({len(backtest.trades)} trades)
      </text>
      {"".join(parts)}
    </svg>
    """
...
def _candle_index(candles: list[dict], timestamp_prefix: str) -> int | None:
    for index, row in enumerate(candles):
        if row["timestamp"][:16] == timestamp_prefix[:16]:
            return index
        if row["timestamp"][:10] == timestamp_prefix[:10]:
            return index
    return None


def render_trades_on_price(candles: list[dict], backtest: BacktestResult) -> str:
    if not candles or not backtest.trades:
        return "<p>No price or trade data.</p>"

    trade_times = {trade.entry_date[:10] for trade in backtest.trades}
    indices = sorted(
        index for index, row in enumerate(candles) if row["timestamp"][:10] in trade_times
    )
    if not indices:
        indices = list(range(max(0, len(candles) - 120), len(candles)))

    start = max(0, indices[0] - 10)
    end = min(len(candles), indices[-1] + 11)
    ...
    markers: list[str] = []
    for trade in backtest.trades:
        entry_index = _candle_index(candles, trade.entry_date)
        if entry_index is None or entry_index < start or entry_index >= end:
            continue
        x = x_for_global_index(entry_index)
        entry_y = y_for_price(trade.entry_price)
        exit_y = y_for_price(trade.exit_price)
        color = "#2ca02c" if trade.return_pct >= 0 else "#d62728"
        side_label = "L" if trade.side == "long" else "S"
        markers.append(
            f'<circle cx="{x:.1f}" cy="{entry_y:.1f}" r="5" fill="#1f77b4" '
            f'stroke="white" stroke-width="1"/>'
            f'<circle cx="{x:.1f}" cy="{exit_y:.1f}" r="5" fill="{color}" '
            f'stroke="white" stroke-width="1"/>'
            f'<line x1="{x:.1f}" y1="{entry_y:.1f}" x2="{x:.1f}" y2="{exit_y:.1f}" '
            f'stroke="{color}" stroke-width="1.5" stroke-dasharray="3,3" opacity="0.7"/>'
            f'<text x="{x + 6:.1f}" y="{entry_y - 8:.1f}" fill="#333" font-size="10">{side_label}</text>'
        )

        level_y = y_for_price(trade.liquidity_price)
        tp1_y = y_for_price(trade.take_profit_1)
        tp2_y = y_for_price(trade.take_profit_2)
        markers.append(
            f'<line x1="{max(0, x - 24):.1f}" y1="{level_y:.1f}" x2="{min(width, x + 24):.1f}" '
            f'y2="{level_y:.1f}" stroke="#ff7f0e" stroke-width="1" stroke-dasharray="2,2" opacity="0.9"/>'
            f'<line x1="{max(0, x - 24):.1f}" y1="{tp1_y:.1f}" x2="{min(width, x + 24):.1f}" '
            f'y2="{tp1_y:.1f}" stroke="#2ca02c" stroke-width="1" stroke-dasharray="3,3" opacity="0.8"/>'
            f'<line x1="{max(0, x - 24):.1f}" y1="{tp2_y:.1f}" x2="{min(width, x + 24):.1f}" '
            f'y2="{tp2_y:.1f}" stroke="#9467bd" stroke-width="1" stroke-dasharray="3,3" opacity="0.8"/>'
        )
    ...
    <svg width="{width}" height="{height}" viewBox="0 0 {width} {height}">
      ...
      <text x="4" y="14" fill="#666" font-size="12">
        Trades on price — blue = entry, green/red = exit, orange = swept liquidity, green dash = +5 pts, purple dash = +15 pts
      </text>
...
def render_price_history(candles: list[dict], resolution: str) -> str:
    if not candles:
    ...
    return _svg_line_chart(
        closes,
        stroke="#1f77b4",
        title=f"{resolution} close ({candles[0]['timestamp'][:10]} → {candles[-1]['timestamp'][:10]})",
        y_suffix=" USD",
    )
