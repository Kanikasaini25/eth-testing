...
def send_test_email() -> EmailResult:
    ...
    return notify_event(
        "Hammer / Shooting Star — test email",
        [
            "This is a test from the Hammer / Shooting Star live runner.",
            f"Recipients configured: {len(recipients)}",
            "You will get mail on buy/sell signals, entries, scale-outs, and closes.",
        ],
    )
