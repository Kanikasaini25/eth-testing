...
def default_poc_rules_path() -> Path:
    configured = get_env("STRATEGY_M15_RULES")
    if configured:
...
def load_poc_rule(path: Path | None = None) -> TradingRule:
    rules_path = path or default_poc_rules_path()
...
class LivePocRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self.state_path = state_path or LIVE_STATE_DIR / f"{self.symbol}_m15_live_state.json"
    ...
    def _execute_entry(
        ...
        self._send_email(
            send_entry_signal_email(
                ...
                strategy_name="15m Previous-Day POC",
            )
        )
        return [action]
