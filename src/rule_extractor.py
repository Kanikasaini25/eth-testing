...
def _extract_liquidity_strategy_rules(text: str) -> list[TradingRule]:
    ...
    return [
        TradingRule(
            ...
            exit_rules=[
                "Target 1: book partial profit when ETH moves 5 points in your favor.",
                "At target 1, exit 80 lots only — do NOT close the full 100 lots.",
                "Keep 20 lots as runner with a trailing stop (default trail: 3 points behind peak).",
                "Trailing stop activates after partial — runner rides the move, not cut at target 1.",
                "Target 2 (runner): nearest swing high/low, or trailing stop exit if price reverses.",
            ],
            risk_rules=[
                ...
                "After partial profit at 5 points, use trailing stop on the 20-lot runner (not full exit).",
                "Trailing stop never moves below breakeven on longs / above breakeven on shorts.",
