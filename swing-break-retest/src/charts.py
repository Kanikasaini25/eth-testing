...
def render_trades_on_price(candles: list[dict], backtest: BacktestResult) -> str:
    ...
    for trade in backtest.trades:
        ...
        level_y = y_for_price(trade.broken_level)
        markers.append(
            f'<line x1="{max(0, x - 20):.1f}" y1="{level_y:.1f}" x2="{min(width, x + 20):.1f}" '
            f'y2="{level_y:.1f}" stroke="#ff7f0e" stroke-width="1" stroke-dasharray="2,2" opacity="0.8"/>'
        )
    ...
    <svg width="{width}" height="{height}" viewBox="0 0 {width} {height}">
      ...
      <text x="4" y="14" fill="#666" font-size="12">
        Trades on 5M price — blue = entry, green/red = exit, orange dash = broken swing level
      </text>
...
def render_price_history(candles: list[dict]) -> str:
    if not candles:
    ...
    return _svg_line_chart(
        closes,
        stroke="#1f77b4",
        title=f"5M close price ({candles[0]['timestamp'][:10]} → {candles[-1]['timestamp'][:10]})",
        y_suffix=" USD",
    )
